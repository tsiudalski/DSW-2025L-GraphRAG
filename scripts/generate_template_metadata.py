...
def get_param_info(param_type: Type) -> str:
    ...
    if hasattr(param_type, "__doc__") and param_type.__doc__:
        doc_lines = [
            line.strip() for line in param_type.__doc__.split("\n") if line.strip()
        ]
        if doc_lines:
            ...
            if len(doc_lines) > 1:
                info_parts.append(
                    " ".join(doc_lines[1:])
                )  # Append subsequent lines as general info
    ...
    if hasattr(param_type, "regex_pattern") and isinstance(
        param_type.regex_pattern, str
    ):
        info_parts.append(f"Matches regex: '{param_type.regex_pattern}'.")
    elif hasattr(param_type, "DEVICE_REGEX") and hasattr(
        param_type.DEVICE_REGEX, "pattern"
    ):
        info_parts.append(f"Matches regex: '{param_type.DEVICE_REGEX.pattern}'.")
    elif hasattr(param_type, "FLOOR_ID_REGEX") and hasattr(
        param_type.FLOOR_ID_REGEX, "pattern"
    ):
        info_parts.append(f"Matches regex: '{param_type.FLOOR_ID_REGEX.pattern}'.")
    elif hasattr(param_type, "FULL_TIMESTAMP_REGEX") and hasattr(
        param_type.FULL_TIMESTAMP_REGEX, "pattern"
    ):
        info_parts.append(
            f"Matches regex: '{param_type.FULL_TIMESTAMP_REGEX.pattern}'."
        )
            ...
            for field_name, field_info in template_class.model_fields.items():
                ...
                description = (
                    ...
                    or (
                        ...
                        and field_info.json_schema_extra.get("description")
                    )
                    or ""
                )
