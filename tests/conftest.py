...
def pytest_html_report_title(report):
    ...
    template_accuracy = (
        (template_tests_passed / template_tests_total * 100)
        if template_tests_total > 0
        else 0
    )
    param_accuracy = (
        (param_tests_passed / param_tests_total * 100) if param_tests_total > 0 else 0
    )
    sparql_accuracy = (
        (sparql_tests_passed / sparql_tests_total * 100)
        if sparql_tests_total > 0
        else 0
    )
